...
def multiply_in_finite_field(a, b):
    m = 0
    polynomial = 0b10011  # binary form of the polynomial x^4 + x + 1

    a = int(a, 2)
    b = int(b, 2)

    while b > 0:
        if b & 1: # least significant bit is set in b
            m ^= a

        a <<= 1  # left shift a by 1 

        if a & 0b10000:  # if the fourth bit is set in a
            a ^= polynomial  

        b >>= 1  # Right shift b by 1 

    return format(m, '04b')  # formating the result as a 4-bit binary
...
# Decryption of cipher text in hexedecimal from file    
def deliverable_3 ( filename ):
    ...
    # removing the padding from the last pair
    decrypted_text[-1] = decrypted_text[-1].replace('0', '')
...
# QUESTION 4
# Problems in above encyrption and decryption scheme
# Pocket AES works on key of 8 bits only which makes only 256 possible keys and it is easy to be breaked by brute force attack
# This scheme uses the same key for encrypting all the blocks of the text and which shows the patterns makes it vulnerable to  attack
# Same key causes the reptition of the same cipher text for the same plain text which makes it vulnerable to attack
# Frequency analysis can be used to break the cipher text with the help of same key as we know some of the letters have higher frequency than others from which we can guess the cipher text
